...
def check(cfg: Config) -> list[str]:
    ...
    # 2. Ollama reachable + embedding model present
    try:
        import ollama

        models = ollama.list().get("models", [])
        names = {m.get("model") or m.get("name") for m in models}
        if not any((cfg.embed_model in (n or "")) for n in names):
            problems.append(
                f"Ollama is up but embedding model {cfg.embed_model!r} is not pulled. "
                f"Run: ollama pull {cfg.embed_model}"
            )
    except Exception as e:  # noqa: BLE001
        problems.append(
            f"Cannot reach Ollama ({e}). Start it with `ollama serve` and "
